...
router.post('/user/bloodtype/:bloodType', auth, async (req, res, next) => {
    try {
        ...
        const { bloodType } = req.params;
        const { _id } = req.user;
        const notAllowedProducts = await (await Product.find({ ["groupBloodNotAllowed." + bloodType]: { $eq: true }}, '-__v ', {limit: 50, sort: { calories: -1}}))
        ...
        const result = await User.findByIdAndUpdate(_id, { parameters, notAllowedProducts} , { new: true });
        if (!result) {
            createError(404);
        }
        const response = {
